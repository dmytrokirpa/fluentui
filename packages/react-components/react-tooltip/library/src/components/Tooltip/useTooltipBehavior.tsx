...
import type { TooltipProps, TooltipChildProps, OnVisibleChangeData } from './Tooltip.types';
import { arrowHeight, tooltipBorderRadius } from './private/constants';
import { useTooltipTimeout } from './private/useTooltipTimeout';
import { Escape } from '@fluentui/keyboard-keys';

type TooltipBehaviorProps = Omit<TooltipProps, 'appearance' | 'content'> & {
  content: React.ComponentProps<'div'>;
}

type TooltipBehaviorState = Required<Omit<TooltipBehaviorProps, 'onVisibleChange' | 'children' | "content" | 'withArrow'>> & {
 ...
 */
export const useTooltipBehavior_unstable = (props: TooltipBehaviorProps): TooltipBehaviorState => {
  'use no memo';
  ...
  const state: TooltipBehaviorState = {
    positioning,
    showDelay,
    hideDelay,
    relationship,
  ...
  state.contentProps.ref = useMergedRefs(state.contentProps.ref, containerRef);
  state.arrowProps.ref = arrowRef;
  ...
  state.contentProps.onPointerEnter = mergeCallbacks(state.contentProps.onPointerEnter, clearDelayTimeout);
  state.contentProps.onPointerLeave = mergeCallbacks(state.contentProps.onPointerLeave, onLeaveTrigger);
  state.contentProps.onFocus = mergeCallbacks(state.contentProps.onFocus, clearDelayTimeout);
  state.contentProps.onBlur = mergeCallbacks(state.contentProps.onBlur, onLeaveTrigger);
  ...
  if (relationship === 'label') {
    ...
    if (typeof state.contentProps.children === 'string') {
      triggerAriaProps['aria-label'] = state.contentProps.children;
    } else {
      triggerAriaProps['aria-labelledby'] = state.contentProps.id;
      // Always render the tooltip even if hidden, so that aria-labelledby refers to a valid element
      state.shouldRenderTooltip = true;
    }
  } else if (relationship === 'description') {
    triggerAriaProps['aria-describedby'] = state.contentProps.id;
