...
import type { Announce, AnnounceOptions, LiveMessage } from '@fluentui/react-toast';
...
export const AriaLive = ({ announceRef }: AriaLiveProps): React.ReactNode => {
  ...
  const announce = useEventCallback((message: string, options: AnnounceOptions) => {
    ...
    const liveMessage: LiveMessage = { message, politeness, createdAt: order.current++ };
    if (!currentMessage) {
